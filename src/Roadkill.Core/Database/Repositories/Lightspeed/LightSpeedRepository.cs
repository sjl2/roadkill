...
namespace Roadkill.Core.Database.LightSpeed
{
	public class LightSpeedRepository : Roadkill.Core.Database.IRepository
	{
	    ...
	    internal IQueryable<TemplateEntity> Templates
	    {
	        get
	        {
	            return UnitOfWork.Query<TemplateEntity>();
	        }
	    }
        ...
        #region ITemplateRepository
        
        public Template AddNewTemplate(Template template)
        {
            TemplateEntity templateEntity = new TemplateEntity();
            ToEntity.FromTemplate(template, templateEntity);
            templateEntity.Id = 0;
            UnitOfWork.Add(templateEntity);
            UnitOfWork.SaveChanges();
        ...
        {
            List<TemplateEntity> entities = Templates.ToList();
            return FromEntity.ToTemplateList(entities);
        }

        public void DeleteTemplate(Template template)
        {
            TemplateEntity entity = UnitOfWork.FindById<TemplateEntity>(template.Id);
            UnitOfWork.Remove(entity);
            UnitOfWork.SaveChanges();
        }
        ...
        {
            UnitOfWork.Remove(new Query(typeof(TemplateEntity)));
            UnitOfWork.SaveChanges();
        }

        public Template GetTemplateById(int id)
        {
            TemplateEntity entity = Templates.FirstOrDefault(p => p.Id == id);
            return FromEntity.ToTemplate(entity);
        }

        public Template GetTemplateByName(string name)
        {
            TemplateEntity entity = Templates.FirstOrDefault(p => p.Name.ToLower() == name.ToLower());
            return FromEntity.ToTemplate(entity);
        }
        ...
        public Template SaveOrUpdateTemplate(Template template)
        {
            ...
            if (entity == null)
            {
                ...
                ToEntity.FromTemplate(template, entity);
                UnitOfWork.Add(entity);
                UnitOfWork.SaveChanges();

                template = FromEntity.ToTemplate(entity);
            }
            else
            {
                ToEntity.FromTemplate(template, entity);
